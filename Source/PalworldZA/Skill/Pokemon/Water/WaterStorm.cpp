...
#include "Skill/Pokemon/Water/WaterStorm.h"
...
AWaterStorm::AWaterStorm()
{
	static ConstructorHelpers::FClassFinder<AProjectileBase> WaterStormRef(TEXT("/Game/BluePrint/PokemonSkill/Water/BP_WaterStormProjectile.BP_WaterStormProjectile_C"));

	if (WaterStormRef.Succeeded())
	{
		WaterStormlClass = WaterStormRef.Class;
	}
}

void AWaterStorm::ExecuteSkill()
{
	if (!User) { return; }
	...
	// 발사체 생성 및 생명 주기 설정
	WaterStorm = SpawnProjectile(UserPos, Rotation);
...
AProjectileBase* AWaterStorm::SpawnProjectile(FVector Pos, FRotator Rot)
{
